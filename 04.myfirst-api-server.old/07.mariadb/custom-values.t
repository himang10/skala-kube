...
metrics:
  ...
  resources:
    ...
    requests:
      cpu: 50m
      memory: 64Mi
  # ServiceMonitor 활성화 (Prometheus가 MariaDB 메트릭 수집)
  # 주의: ServiceMonitor CRD가 설치된 후 enabled: true로 변경
  serviceMonitor:
    enabled: false
    namespace: {{NAMESPACE}}
    labels:
...
## 환경 변수 (OpenTelemetry 지원 추가)
extraEnvVars:
  - name: MARIADB_EXTRA_FLAGS
    value: "--log-bin=mysql-bin --binlog-format=ROW --server-id=1"
  # OpenTelemetry 환경변수 (MariaDB Pod에 Beyla 적용을 위한 라벨링)
  - name: OTEL_SERVICE_NAME
    value: "mariadb"
  - name: OTEL_SERVICE_VERSION
    value: "11.4.6"
  - name: OTEL_RESOURCE_ATTRIBUTES
    value: "service.name=mariadb,service.namespace={{NAMESPACE}},service.version=11.4.6,database.type=mariadb"

## 추가: Pod 라벨 설정 (Beyla가 감지할 수 있도록)
commonLabels:
  app: mariadb
  component: database
